...
export function ShowMenuFilter(url){
  return (permissionList.indexOf(url)>-1||userType == 1);
}
export function majorIdFilter(majorId,majorList){
  let major = '';
...
export function ageFilter(birth_time){
  let fullYear = new Date().getFullYear();
  let oldDate = new Date();
  oldDate.setTime(birth_time);
  let oldYear = oldDate.getFullYear();
  return fullYear - oldYear;
}

export function wordDefaultFilter(wordList){
  let id = 0;
 ...
 * 转大写金额
 */
export function moneyToCapital(num) {
  var fuhao = "";
  var text = num + "";
  if (text.indexOf("-") > -1) {
      num = text.replace("-", "");
